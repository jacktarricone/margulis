## writing the code for multiply each raster swe value by its pixel area
## and then summing the values to get total vol swe 
## started 7/20/2020
...
######function which chunk out analysis
...
#bind on doWY column
full_year <-cbind(doWY_df, results_14)

write.csv(full_year, row.names = FALSE)

return(results)

}

system.time(results_14<-big_vol_fun("SN_SWE_WY2014.h5"))

write.csv(full_year,"/Volumes/john_tarricone/UNR_summer_20/margulis/volume_results/WY_2014_SN_ts.csv", row.names = FALSE)

ggplot(full_year)+ aes(doWY,WY_2014) + 
  geom_line(color="darkblue") +
  scale_x_continuous(breaks = seq(0,366,50)) +
  scale_y_continuous(breaks = seq(0,30,2)) +
  labs(title="WY 2014 CA Sierra Nevada Volumetric SWE", y=(expression('Volumetric SWE'~(km^{"3"}))), x="Day of Water Year") +
  theme_light(11)
